...
from flask import current_app, Blueprint, render_template_string
from pathlib import Path

from .helpers import (
    cfg_get, leds_status, iface_is_up, rec_is_active,
    disk_info, list_media, client_prefers_hls,
    time_info, hw_inventory
)
...
<div class="card">
  ...
  <div class="kv">
    <div><b>RTC</b></div>
    <div>
      Driver: {{ hw.rtc.driver or '-' }},
      Bus: {{ hw.rtc.i2c_bus or '-' }},
      Addr: {{ hw.rtc.i2c_addr or '-' }}<br/>
      Kernel name: <code>{{ hw.rtc.kernel_name or '-' }}</code>
    </div>
    <div><b>Camera</b></div>
    <div>Dev: {{ hw.camera.dev }}, Name: {{ hw.camera.name or '-' }}</div>
    <div><b>Storage</b></div>
    <div>Mount: {{ hw.storage.mount }}, FS: {{ hw.storage.fstype or '-' }},
      Label: {{ hw.storage.label or '-' }}, Model: {{ hw.storage.model or '-' }},
      Size: {{ hw.storage.size or '-' }}</div>
  </div>
</div>
...
<div class="card">
  ...
  <div class="row" style="margin:.6rem 0">
    <form method="post" action="/action/format"
          onsubmit="return confirm('Xoá TẤT CẢ file video trong thư mục ghi hình?');">
      <input type="hidden" name="confirm" value="YES">
      <button class="danger">Format (xoá file)</button>
    </form>
    <form method="post" action="/action/reset"
          onsubmit="return confirm('Reset factory?');">
...
.card{background:var(--card);border:1px solid var(--bd);border-radius:14px;padding:16px;margin-bottom:14px;box-shadow:0 1px 8px rgba(0,0,0,.04)}
h1{margin:.2rem 0 1rem;font-size:1.6rem}
h2{margin:.4rem 0 .8rem;font-size:1.1rem}
.kv{display:grid;grid-template-columns:160px 1fr;gap:.4rem 1rem}
.leds{display:flex;gap:10px;flex-wrap:wrap}
.led{display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:9999px;border:1px solid var(--bd);background:#fff}
.dot{width:10px;height:10px;border-radius:9999px;background:var(--off)}
.on .dot{background:var(--ok)}
.blink .dot{animation:bl 1s linear infinite;background:var(--warn)}
@keyframes bl{0%{opacity:1}50%{opacity:.15}100%{opacity:1}}
.muted{color:var(--muted)}
.row{display:flex;gap:10px;flex-wrap:wrap}
button{border:1px solid var(--bd);background:#fff;padding:8px 12px;border-radius:10px;cursor:pointer}
